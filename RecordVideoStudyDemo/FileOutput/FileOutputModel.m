...
@interface FileOutputModel ()<AVCaptureFileOutputRecordingDelegate>
...
@property (nonatomic, assign) FileCameraFlashState flashState;

@end

@implementation FileOutputModel
...
- (void)setRecordState:(FileRecordState)recordState
{
...
#pragma mark - setup
- (void)setUpWithType:(FileVideoViewType )type
{
...
- (void)setUpVideo
{
    ...
    // 1.2 创建视频输入源
    NSError *error=nil;
    self.videoCaptureInput= [[AVCaptureDeviceInput alloc] initWithDevice:videoCaptureDevice error:&error];
    // 1.3 将视频输入源添加到会话
...
- (void)setUpPreviewLayerWithType:(FileVideoViewType )type
{
    CGRect rect = CGRectZero;
    switch (type) {
        case FileVideoType1X1:
            rect = CGRectMake(0, 0, KSCREENWIDTH, KSCREENWIDTH);
            break;
        case FileVideoType4X3:
            rect = CGRectMake(0, 0, KSCREENWIDTH, KSCREENWIDTH*4/3);
            break;
        case FileVideoTypeFullScreen:
            rect = _superView.frame;
            break;
        default:
            rect = _superView.frame;
            break;
...
-(void)changeDeviceProperty:(void(^)(AVCaptureDevice *captureDevice))propertyChange{
    AVCaptureDevice *captureDevice= [self.videoCaptureInput device];
    NSError *error;
...
{
    if(_flashState == FileCameraFlashClose){
        if ([self.videoCaptureInput.device hasTorch]) {
            ...
            _flashState = FileCameraFlashOpen;
        }
    }else if(_flashState == FileCameraFlashOpen){
        if ([self.videoCaptureInput.device hasTorch]) {
            [self.videoCaptureInput.device lockForConfiguration:nil];
            [self.videoCaptureInput.device setTorchMode:AVCaptureTorchModeAuto];
            [self.videoCaptureInput.device unlockForConfiguration];
            _flashState = FileCameraFlashAuto;
        }
    }else if(_flashState == FileCameraFlashAuto){
        if ([self.videoCaptureInput.device hasTorch]) {
            [self.videoCaptureInput.device lockForConfiguration:nil];
            [self.videoCaptureInput.device setTorchMode:AVCaptureTorchModeOff];
            [self.videoCaptureInput.device unlockForConfiguration];
            _flashState = FileCameraFlashClose;
        }
    };
...
{
    self.recordState = FileRecordStateInit;
    _recordTime = 0;
    [self.session startRunning];
}
...
- (void)setUpInit
{
    ...
    [self clearFile];
    _recordTime = 0;
    _recordState = FileRecordStateInit;
...
- (void)refreshTimeLabel
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(updateRecordingProgress:)]) {
        [self.delegate updateRecordingProgress:_recordTime/RECORDMAXDUR];
    }
    if (_recordTime >= RECORDMAXDUR) {
        [self stopFileRecordVideo];
    }
}
...
{
    self.recordState = FileRecordStateRecording;
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    if ([ZHFileManager isExistsAtPath:[self.videoUrl path]]) {
        
        self.recordState = FileRecordStateFinish;
        //剪裁成正方形
